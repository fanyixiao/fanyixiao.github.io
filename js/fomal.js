...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | fanyixiao")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'D3PBZ-4LM6B-5FNU2-JOGBJ-GW6BK-U5F7Z',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime1() {
  var grt = new Date("10/21/2022 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by fanyixiao %c 你正在访问灯塔-beacon",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 灯塔") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('春草梦，也宜夏的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("10/21/2023 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("10/21/2023 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://cdn.jsdelivr.net/gh/fanyixiao/fanyixiao_img-bed@main/img/p1.webp);
    --darkmode-bg:url(https://cdn.jsdelivr.net/gh/fanyixiao/fanyixiao_img-bed@main/img/p1.webp);
    --mobileday-bg: url(https://cdn.jsdelivr.net/gh/fanyixiao/fanyixiao_img-bed@main/img/p2.webp);
    --mobilenight-bg: url(https://cdn.jsdelivr.net/gh/fanyixiao/fanyixiao_img-bed@main/img/p2.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
